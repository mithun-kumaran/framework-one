...
val PageContentWideStyle = CssStyle {
    base { Modifier.fillMaxSize().padding(leftRight = 2.cssRem, top = 1.5.cssRem, bottom = 2.5.cssRem) }
}
...
fun PageLayout(ctx: PageContext, content: @Composable ColumnScope.() -> Unit) {
    ...
    val gridModifier = if (data.showFooter) {
        Modifier.gridTemplateRows { size(1.fr); size(minContent) }
    } else {
        Modifier.gridTemplateRows { size(1.fr) }
    }
